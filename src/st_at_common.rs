...
pub const COMMON_AT_BEST_3: [CommonAtType; 4] = [CommonAtType::AT4PN, CommonAtType::ATUPX, CommonAtType::AT3PX, CommonAtType::PKDPX];
pub const COMMON_AT_BEST_4: [CommonAtType; 4] = [CommonAtType::AT4PN, CommonAtType::ATUPX, CommonAtType::AT4PX, CommonAtType::PKDPX];
pub const COMMON_AT_MUST_COMPRESS_3: [CommonAtType; 3] = [CommonAtType::ATUPX, CommonAtType::AT3PX, CommonAtType::PKDPX];
pub const COMMON_AT_MUST_COMPRESS_4: [CommonAtType; 3] = [CommonAtType::ATUPX, CommonAtType::AT4PX, CommonAtType::PKDPX];
...
impl CommonAt {
    ...
    pub fn compress<'a, I>(uncompressed_data: &[u8], compression_type: I) -> PyResult<Vec<u8>> where I: Iterator<Item=&'a CommonAtType> {
        ...
        for t in compression_type {
            match t {
                CommonAtType::AT4PN =>  Self::compress_try(At4pn::compress(uncompressed_data).map(|x| x.to_bytes()), &mut meta),
                CommonAtType::AT3PX => Self::compress_try(At3px::compress(uncompressed_data).map(|x| x.to_bytes()), &mut meta),
                CommonAtType::AT4PX => Self::compress_try(At4px::compress(uncompressed_data).map(|x| x.to_bytes()), &mut meta),
                CommonAtType::ATUPX => {
                    if env::var("SKYTEMPLE_ALLOW_ATUPX").is_ok() {
                        Self::compress_try(Atupx::compress(uncompressed_data).map(|x| x.to_bytes()), &mut meta);
                    }
                },
                CommonAtType::PKDPX => Self::compress_try(Pkdpx::compress(uncompressed_data).map(|x| x.to_bytes()), &mut meta),
            }
        }
        match meta.0 {
            Some(x) => Ok(x),
